...
class RequestHandler(object):
    ...
    def __init__(self, application, request, **kwargs):
        ...
        self._headers_written = False       # todo:这是个什么鬼?
        self._finished = False
        self._auto_finish = True
        self._transforms = None  # will be set in _execute, todo: 这什么?
    ...
    def clear(self):
        """Resets all(重置所有 response的headers和content)
        headers and content for this response."""
    ...
    def set_header(self, name, value):
        """设置给定的响应头名称和值.
        如果日期时间给出，我们会自动根据HTTP规范进行格式化。
        如果该值不是字符串，我们把它转换为一个字符串。所有标头值，然后编码为UTF-8。
        """
    ...
    def add_header(self, name, value):
        ...
        不像`set_header`, `add_header`可以被多次调用去返回多个值相同的响应头
        """
    ...
    def write(self, chunk):
        """Writes the given chunk(块) to the output buffer(缓冲区).

        To write the output to the network, use the flush() method below.

        字典-->Json 并设置Content-Type:application/json
        ...
        list类型不会自动转换成json，因为防御`cross-site security`, 所有输出必须包装成字典
        ...
        """
        if self._finished:
            # 如果完成了_finished状态(True)则触发异常
            raise RuntimeError("Cannot write() after finish().  May be caused "
                               "by using async operations without the "
                               "@asynchronous decorator.")
        if not isinstance(chunk, (bytes_type, unicode_type, dict)):
            # 发送的chunk类型为`bytes_type`,`unicode_type`或`dict`
            raise TypeError("write() only accepts bytes, unicode, and dict objects")
        if isinstance(chunk, dict):
            # 这里用到了tornado自己封装的json库
            chunk = escape.json_encode(chunk)
            # 设置头部json，注意`charset=UTF-8`输出格式
            self.set_header("Content-Type", "application/json; charset=UTF-8")
        chunk = utf8(chunk)     # 设置utf-8格式

        # 写入缓冲区,注意这里缓冲区`_write_buffer`是列表形式，把块append其中，等后面`finish`一并输出。
    ...
    def render_string(self, template_name, **kwargs):
        ...
        生成给定模板通过给定参数，返回utf8格式的byte str
    ...
    def flush(self, include_footers=False, callback=None):
        ...
        把缓冲区里的数据“立即”写到输出流中去,一般在关闭或者完成输入之前都要调用保证数据立即得到传输。
        输入输出流一般都是用缓冲的，也就是说不是每次你要它读/写一个或少数几个字节，他就立马执行的，
        而是将这些请求放入缓冲，积累到一定量的时候才一起执行，以提高效率。
        flush的作用就是强制将缓冲区里的东西立刻执行然后清空，不管里面有多少东西。
        参考：http://bbs.csdn.net/topics/30040187

        `callback`用于流控制，当所有`flushed data`被写入到`socket`后则里面执行该callback

        The ``callback`` argument, if given, can be used for flow control:
        it will be run when all flushed data has been written to the socket.

        注意：一个标准输出在同一时间内只有一个`flush callback`

        Note that only one flush callback can be outstanding at a time;
        if another flush occurs before the previous flush's callback
        has been run, the previous callback will be discarded(丢弃).
        ...
        """
        # todo: 还是比较陌生
        chunk = b"".join(self._write_buffer)        # 转换成字节str
        self._write_buffer = []
    ...
    def send_error(self, status_code=500, **kwargs):
        ...
        an error, so this method will simply terminate(终结) the response.

        If output has been written but not yet flushed, it will be discarded(废弃)
        and replaced with the error page.

        重写`write_error()` 自定义错误页面

        Override `write_error()` to customize the error page that is returned.

        额外关键字**kwargs 用于传入`write_error`方法中
    ...
    def data_received(self, chunk):
        ...
        实现此方法来处理流请求数据,需要`.stream_request_body` 装饰器.
        """
...
class URLSpec(object):
    """指定的URLs和handlers之间的映射"""
    def __init__(self, pattern, handler, kwargs=None, name=None):
        """Parameters:

        * ``pattern``: 要匹配的正则表达式.
          任何在正则里的组(groups)将被传递到get/post等方法作为参数

        * ``handler``: `RequestHandler` 子类的调用.

        * ``kwargs`` (optional): 额外的参数字典,要传递到handler的构造。

        * ``name`` (optional): 这个handler的名称.被`Application.reverse_url`使用
        """
...
def create_signed_value(secret, name, value, version=None, clock=None):
    ...
    if version == 1:
        # 版本1是按照 secretKey, name, value, 时间戳组合，`|`分割
